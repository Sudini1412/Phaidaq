...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Low-level interface for midas I/O')
    parser.add_argument("input", help="path to midasfile", nargs='+')
#    parser.add_argument("-o", "--output", type=str, help="write output to WAV format", required=False)
    parser.add_argument("-p", "--plot", action="store_true", help="plot waveforms")
#    parser.add_argument("-s", "--sum", action="store_true", help="sum up waveforms")
    parser.add_argument("-m", "--maxevents", type=int, help="maximum number of QDC events to read out (0 means ALL)", default=0)
    parser.add_argument("-e", "--expt", type=str,help="select the experiment", default="NAsetup1", choices=["proto0","dart","NAsetup1","NAsetup2"])
    ...
    if args.plot:
        ...
        axs=ax.flatten()
    print(args.input)
    mread = MIDASreader(args.input,MIDASconf[args.expt]["ADC"],MIDASconf[args.expt]["BANKS"])
    for i,event in enumerate(mread):
        if event is None: continue
        print(f'Event # {i}\t#Modules {event.nboards}\t#Channels {event.nchannels}\t#Samples {event.nsamples}')
        wfs=event.adc_data
        print(f'\tchannel mask {bin(event.channel_mask)[2:]}\ttrigger time: {event.trigger_time}')
        if args.plot and event.nchannels > 0:
            timebin=MIDASconf[args.expt]["bin"]
            t=np.arange(0.,wfs.shape[1]*timebin,timebin)
            nsubp=min(wfs.shape[0],16)
            for ch in range(nsubp):
                axs[ch].plot(t,wfs[ch,])
        if i>=args.maxevents and args.maxevents>0: break
    if args.plot: 
        fig.tight_layout()
        plt.show()
